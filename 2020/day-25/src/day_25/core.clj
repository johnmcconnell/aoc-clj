...
(defn path->xy
  [path]
  (let [parts (re-seq #"se|sw|ne|nw|e|w" path)
        init-coord [0 0]
        translate {"se" [1 -1]
                   "sw" [-1 -1]
                   "ne" [1 1]
...
(defn find-loop
  ([s needle] (find-loop s needle (* 10 1000 1000)))
  ([s needle l]
   (loop [i 0
          v 1]
     (cond
...
(defn find-enc-key
  ...
  (let [[lp1 lp2] (->>
                       s-coll
                       (map (fn [[s n]] (find-loop s n)))
                       (mapv first))
        [[_ n1] [_ n2]] s-coll]
    [[(transform n1 lp2) lp2]
     [(transform n2 lp1) lp1]]))

(defn find-enc-key-2
  [coll]
  (let [lm (* 1000 1000)]
    (loop [s 2]
      (when (zero? (mod s (quot lm 100)))
        (prn s))
      (if (< s lm)
        (let [[lp1 lp2] (->>
                         coll
                         (map (fn [n] (find-loop s n)))
                         (mapv first))
              [n1 n2] coll]
          (if (or (nil? lp1) (nil? lp2))
            (recur (inc s))
            [[(transform n1 lp2) lp2]
             [(transform n2 lp1) lp1]]))
        [nil s]))))
...
(comment
  ...
  (find-loop 7 5764801)
  (find-enc-key [[7 5764801] [7 17807724]])
  (find-enc-key [[7 10705932] [7 12301431]])
  (find-enc-key-2 [10705932 12301431])
  (def x *1)
  ...
  (re-seq #"se|sw|ne|nw|e|w" "sesenwnenenewseeswwswswwnenewsewsw" )
)
