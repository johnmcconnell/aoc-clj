...
(defn game-loop
  ([coll] (game-loop coll 100000))
  ([coll limit]
   (loop [turn 0
          coll coll]
     (cond
       (= turn limit) [coll turn]
       (some (comp empty? ::deck) coll) [coll
                                         turn
                                         (->> coll (remove (comp empty? ::deck)) first)]
       :else (let [decks (mapv ::deck coll)
             cards (mapv first decks)
             decks* (mapv (comp vec rest) decks)
             decks* (if (apply < cards)
                      (update decks* 1 (fn [deck] (apply conj deck (reverse cards))))
                      (update decks* 0 (fn [deck] (apply conj deck cards))))]
               (recur
                (inc turn)
                (mapv
                 (fn [p deck] (assoc p ::deck deck))
                 coll
                 decks*)))))))
...
(defn score-deck
  ...
  (reduce
   (fn [ttl [i v]]
     (+ ttl (* (inc i) v)))
   0
   (map-indexed vector (reverse deck))))

(defn subgame
  [f coll game limit memo cards decks]
  (let [subdecks (mapv (fn [card deck] (subvec deck 0 card)) cards decks)
        [i] (f (mapv (fn [p deck] (assoc p ::deck deck)) coll subdecks) (inc game) limit memo)
        ;; _ (prn "Subgame winner: " (inc game) i)
        cards (if (= i 0)
                cards
                (-> cards reverse vec))]
    [i cards]))
...
(defn game-loop-2
  ([coll] (game-loop-2 coll 1))
  ([coll game] (game-loop-2 coll game 1e6))
  ([coll game limit] (game-loop-2 coll game limit #{}))
  ([coll game limit memo]
   (loop [turn 0
          coll coll
          memo memo]
     (when (and (not= turn 0) (zero? (mod turn 10000)))
       (prn game turn (count memo) coll))
     (cond
       (contains? memo coll) [0 coll turn (assoc (get coll 0) ::victory ::memo)]
       (= turn limit) [coll turn]
       (some (comp empty? ::deck) coll) [(->>
                                          coll
                                          (map-indexed vector)
                                          (remove (comp empty? ::deck second))
                                          first
                                          first)
                                         coll
                                         turn]
       :else (let [decks (mapv ::deck coll)
                   cards (mapv first decks)
                   decks* (mapv (comp vec rest) decks)
                   subgame? (every? (fn [[card deck]] (<= card (count deck))) (map vector cards decks*))
                   [i cards] (if subgame?
                               (subgame game-loop-2 coll game limit #{} cards decks*)
                               (if (apply < cards)
                                 [1 (-> cards reverse vec)]
                                 [0 cards]))
                   decks* (update decks* i (fn [deck] (apply conj deck cards)))
                   coll* (mapv
                          (fn [p deck] (assoc p ::deck deck))
                          coll
                          decks*)]
...
(comment
  (def a-input
    (let [part :a
      ...
      coll))
  a-input
  (game-loop-2 a-input 0)
  (time (game-loop-2 a-input 0))
  ;; "Elapsed time: 111787.608504 msecs"
  (def x *1)
  x
  (get-in x [1 1])
  (score-deck (get-in x [1 1]))
  (score-deck (nth (game-loop a-input) 2))
  ...
  (def c-input
    (let [part :c
      ...
      coll))
  c-input
  (game-loop c-input 30)
  (score-deck (nth (game-loop c-input) 2))

  (game-loop-2 c-input 0 17)
