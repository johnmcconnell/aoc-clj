...
#_(defn assoc-neighbors
  [tiles]
  (let [matched-edges (fn [t1 t2]
                        (if (= (::id t1) (= (::id t2)))
                          nil
                          (let [ed1 (::edges t1)
                                ed2 (::edges t2)
                                edges (for [[dir1 e1] ed1
                                            [flip1 v1] e1
                                            [dir2 e2] ed2
                                            [flip2 v2] e2]
                                        (if (= v1 v2)
                                          [[dir1 flip1] [t2 dir2 flip2]]
                                          nil))
     ...
     (fn [tiles* {::keys [id edges]
       ...
       (let [neighbors (->
                        (mapcat
                         ...
                         tiles))
             nbrs (reduce
                   (fn [v [[dir1 flip1] [t1 dir2 flip2]]]
                     (update-in v [dir1 flip2] (fnil conj #{}) [t1 dir2 flip2]))
                   nil
                   neighbors)
             tile (assoc tile ::neighbors nbrs)]
         (conj tiles* tile)))
     nil
     tiles)))
...
(defn debug-tiles
  ...
  (mapv
   (fn [tile]
     (->
      tile
      (update ::neighbors #(reduce + 0 (map count (vals %1))))
      (dissoc ::edges)
      )
     )
   coll))
...
(defn select-tile
  [coll id]
  (filter (comp #(= id %) ::id) coll))

(defn find-two-edge-tiles
  [tiles]
  (let [edges (->>
               tiles
               (mapcat
                (fn [{::keys [edges]}]
                  (vector
                   ...
                   (::left edges)
                   (::right edges)))))
        freqs (frequencies edges)
        edge->tiles (reduce
                     (fn [m {::keys [edges id]}]
                       (reduce
                        (fn [m k]
                          (update m (k edges) (fnil conj #{}) id))
                        m
         ...
         ::edges (->>
                  (mapv
                   (fn [[k v]]
                     [k (get edge->tiles v)])
                   edges))}))
     (filter (comp #(= % {2 2 1 2}) ::edge-freqs)))))
...
(comment
  ...
  (def c-input
    (let [part :c
      ...
      r))
  c-input
  (clojure.pprint/pprint (find-two-edge-tiles c-input))
  (->>
   (find-two-edge-tiles c-input)
   (mapv ::id)
   (apply * ))
  (first c-input)
  (def body (assoc-neighbors c-input))
  body
  (first body)
  (keys (first body))
  ...
  (clojure.pprint/pprint (select-tile body 2311))
  (clojure.pprint/pprint (select-tile body 1951))
