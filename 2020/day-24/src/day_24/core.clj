...
(defn update-tiles
  ...
  (let [v (get s [x y])
        ...
        nbs-cnt (count v-nbs)
        v* (cond
             (and (= v :occupied) (or (zero? nbs-cnt) (> nbs-cnt 2))) nil
             (and (nil? v) (= nbs-cnt 2)) :occupied
             :else v)]
    (if (= v* :occupied)
      (assoc s* [x y] v*)
      s*)))
...
(defn eval-tiles
  ...
  (let [xy-coords (fn
                    ...
                    (reduce
                     (fn [bag [[x y] _]]
                       (-> (apply conj (conj bag [x y]) (xy-search [x y]))))
                     #{}
                     s))]
    (loop [i 0
           s state]
...
(comment
  ...
  a-input
  (flip-tiles a-input)
  (-> (eval-tiles (-> a-input flip-tiles tiles->occupied) 100) count)
  (count *1)
  ...
  (def c-input
    (let [part :c
      ...
      coll))
  c-input
  (-> (eval-tiles (-> c-input flip-tiles tiles->occupied) 100) count)
  (count *1)
